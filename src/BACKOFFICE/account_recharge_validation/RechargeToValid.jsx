...
import { UrlHost } from '../url/UrlHost';
...
const RechargeToValid = () => {
    ...
    useEffect(() => {
        ...
        fetch(new UrlHost().url+'/account_recharges', content)
            .then((response) => {
    ...
    const valid = (row_id,status) => {
        ...
        content.body = JSON.stringify({
            account_recharge_id: row_id,admin:{id: user.user_account.id},status: status
        });

        console.log(content.body);
        fetch(new UrlHost().url+'/account_recharges/'+row_id+'/valid', content)
            .then((response) => {
                if (response.status === 400) {
                    alert('Error');
                } else return response.json();
